...
for i in range(100):
    ...
    model= KNeighborsClassifier(weights= 'uniform',n_neighbors= 8) 
    ...
    score_auc=str(auc)+'\n'           
    df_1=open('kirp-knn-aucs-methy.txt','a')
    df_1.write(score_auc)
    
    score_acc=str(acc)+'\n'           
    df_1=open('kirp-knn-accs-methy.txt','a')
    df_1.write(score_acc)
    
    score_pr_auc=str(pr_auc)+'\n'           
    df_1=open('kirc-knn-prauc-methy.txt','a')
    df_1.write(score_pr_auc)
    
    score_mcc=str(mcc)+'\n'           
    df_1=open('kirp-knn-mcc-methy.txt','a')
    df_1.write(score_mcc)
   
    score_sen=str(sen)+'\n'           
    df_1=open('kirp-knn-sen-methy.txt','a')
    df_1.write(score_sen)
    
    score_sps=str(sps)+'\n'           
    df_1=open('kirp-knn-sps-methy.txt','a')
...
'''
auc-mean-score: 0.684
acc-mean-score: 0.771
pr-mean-score: 0.876
mcc-mean-score: 0.251
sen-mean-score: 0.981

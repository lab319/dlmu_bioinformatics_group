...
for i in range(100):
    ...
    model=SVC(kernel='linear', C=1e-05,random_state=0,probability=True)  
    ...
    score_auc=str(auc)+'\n'           
    df_1=open('kirp-svm-aucs-mirna.txt','a')
    df_1.write(score_auc)
    
    score_acc=str(acc)+'\n'           
    df_1=open('kirp-svm-accs-mirna.txt','a')
    df_1.write(score_acc)
    
    score_pr_auc=str(pr_auc)+'\n'           
    df_1=open('kirp-svm-prauc-mirna.txt','a')
    df_1.write(score_pr_auc)
    
    score_mcc=str(mcc)+'\n'           
    df_1=open('kirp-svm-mcc-mirna.txt','a')
    df_1.write(score_mcc)
   
    score_sen=str(sen)+'\n'           
    df_1=open('kirp-svm-sen-mirna.txt','a')
    df_1.write(score_sen)
    
    score_sps=str(sps)+'\n'           
    df_1=open('kirp-svm-sps-mirna.txt','a')
...
'''
auc-mean-score: 0.844
auc-median-score: 0.852
acc-mean-score: 0.745
acc-median-score: 0.745
pr-mean-score: 0.929
pr-median-score: 0.932
mcc-mean-score: 0.000
mcc-median-score: 0.000
sen-mean-score: 1.000
sen-median-score: 1.000
sps-mean-score: 0.000
sps-median-score : 0.000
'''

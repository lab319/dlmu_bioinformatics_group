...
for i in range(100):
    ...
    model= KNeighborsClassifier(weights='distance',n_neighbors= 4,p=5) 
    ...
    score_auc=str(auc)+'\n'           
    df_1=open('kirp-knn-aucs-mirna.txt','a')
    df_1.write(score_auc)
    
    score_acc=str(acc)+'\n'           
    df_1=open('kirp-knn-accs-mirna.txt','a')
    df_1.write(score_acc)
    
    score_pr_auc=str(pr_auc)+'\n'           
    df_1=open('kirc-knn-prauc-mirna.txt','a')
    df_1.write(score_pr_auc)
    
    score_mcc=str(mcc)+'\n'           
    df_1=open('kirp-knn-mcc-mirna.txt','a')
    df_1.write(score_mcc)
   
    score_sen=str(sen)+'\n'           
    df_1=open('kirp-knn-sen-mirna.txt','a')
    df_1.write(score_sen)
    
    score_sps=str(sps)+'\n'           
    df_1=open('kirp-knn-sps-mirna.txt','a')
...
'''
auc-mean-score: 0.724
acc-mean-score: 0.776
pr-mean-score: 0.897
mcc-mean-score: 0.303
sen-mean-score: 0.952
sps-mean-score: 0.261
'''

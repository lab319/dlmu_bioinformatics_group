...
for i in range(100):
    ...
    score_auc=str(auc)+'\n'           
    df_1=open('kirp-xgboost-aucs-mrna.txt','a')
    df_1.write(score_auc)
    
    score_acc=str(acc)+'\n'           
    df_1=open('kirp-xgboost-accs-mrna.txt','a')
    df_1.write(score_acc)
    
    score_pr_auc=str(pr_auc)+'\n'           
    df_1=open('kirp-xgboost-prauc-mrna.txt','a')
    df_1.write(score_pr_auc)
    
    score_mcc=str(mcc)+'\n'           
    df_1=open('kirp-xgboost-mcc-mrna.txt','a')
    df_1.write(score_mcc)
   
    score_sen=str(sen)+'\n'           
    df_1=open('kirp-xgboost-sen-mrna.txt','a')
    df_1.write(score_sen)
    
    score_sps=str(sps)+'\n'           
    df_1=open('kirp-xgboost-sps-mrna.txt','a')
...
'''
auc-mean-score: 0.863
acc-mean-score: 0.843
pr-mean-score: 0.933
mcc-mean-score: 0.556
sen-mean-score: 0.950
sps-mean-score: 0.530
'''

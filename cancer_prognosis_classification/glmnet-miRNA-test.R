...
library(e1071)
library(caTools)
x<-read.table("KIRP_mirna.txt",sep='\t',header=TRUE)
...
for(i in 0:99)
  {	
	...
	fit = glmnet(x_train, y=y_train, family='binomial', alpha=0.2, lambda= 0.3215656)
	print(fit)
	
	predict<-predict(fit,x_test,type="class",s=0.3215656)
	predictions<-predict.glmnet(fit,x_test,type="class",s=0.3215656)
	predictions

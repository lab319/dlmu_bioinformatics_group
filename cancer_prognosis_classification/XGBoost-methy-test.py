...
for i in range(100):
    ...
    score_auc=str(auc)+'\n'           
    df_1=open('kirp-xgboost-aucs-methy.txt','a')
    df_1.write(score_auc)
    
    score_acc=str(acc)+'\n'           
    df_1=open('kirp-xgboost-accs-methy.txt','a')
    df_1.write(score_acc)
    
    score_pr_auc=str(pr_auc)+'\n'           
    df_1=open('kirp-xgboost-prauc-methy.txt','a')
    df_1.write(score_pr_auc)
    
    score_mcc=str(mcc)+'\n'           
    df_1=open('kirp-xgboost-mcc-methy.txt','a')
    df_1.write(score_mcc)
   
    score_sen=str(sen)+'\n'           
    df_1=open('kirp-xgboost-sen-methy.txt','a')
    df_1.write(score_sen)
    
    score_sps=str(sps)+'\n'           
    df_1=open('kirp-xgboost-sps-methy.txt','a')
...
acc-mean-score: 0.829
pr-mean-score: 0.939
mcc-mean-score: 0.508
sen-mean-score: 0.952
sps-mean-score: 0.468
'''
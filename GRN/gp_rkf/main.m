% the inference of the first equation of E_cell model under Laplace noise by GP and RKF
%
...
popu = gprkf_evaluate(popu,[1:popusize],X,Y,[],opt(6:9));

% info
disp(gprkf_result([],0));       
disp(gprkf_result(popu,1));

% GP+rkf loops
 for c = 2:iteration
  %iterate 
  popu = gprkf_mainloop(popu,X,Y,[],opt);
  disp(gprkf_result(popu,1));
end

% Result: return the optimum solution and polynome in the final generation
[s,tree,func] = gprkf_result(popu,2);
disp(s);

function [popu,evnum] = gprkf_mainloop(popuin,X,Y,Q,opt);
%Run one evolutionary loop, makes the next generation
% [popu,evnum] = gprkf_mainloop(popuin,X,Y,Q,opt)
...
%Selection
selm = gprkf_selection(popuin,ggap,pc,pm,tsels);
...
%Evaulate new indv.s
popu = gprkf_evaluate(popu,newix,X,Y,Q,opt(6:9));

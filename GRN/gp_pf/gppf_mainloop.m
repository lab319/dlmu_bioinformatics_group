function [popu,evnum] = gppf_mainloop(popuin,X,Y,Q,opt);
%Run one evolutionary loop, makes the next generation
% [popu,evnum] = gppf_mainloop(popuin,X,Y,Q,opt)
...
%Selection
selm = gppf_selection(popuin,ggap,pc,pm,tsels);
...
%Evaulate new indv.s
popu = gppf_evaluate(popu,newix,X,Y,Q,opt(6:9));

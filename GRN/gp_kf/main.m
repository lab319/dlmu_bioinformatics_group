% the inference of the first equation of E_cell model under Laplace noise by GP and KF
%
...
popu = gpkf_init(popusize,maxtreedepth,symbols);

% first evaluation
opt = [0.8 0.5 0.3 2 1 0.2 30 0.05 0 0];
popu = gpkf_evaluate(popu,[1:popusize],X,Y,[],opt(6:9));

% info
disp(gpkf_result([],0));       
disp(gpkf_result(popu,1));

% GP+KF loops
 for c = 2:20,
  %iterate 
  popu = gpkf_mainloop(popu,X,Y,[],opt);
  disp(gpkf_result(popu,1));
end

% Result: return the optimum solution and polynome in the final generation
[s,tree,func] = gpkf_result(popu,2);
disp(s);

function [popu,evnum] = gprls_mainloop(popuin,X,Y,Q,opt)
%Run one evolutionary loop, makes the next generation
% [popu,evnum] = gprls_mainloop(popuin,X,Y,Q,opt)
...
%Selection
selm = gprls_selection(popuin,ggap,pc,pm,tsels);
...
%Evaulate new indv.s
popu = gprls_evaluate(popu,newix,X,Y,Q,opt(6:9));

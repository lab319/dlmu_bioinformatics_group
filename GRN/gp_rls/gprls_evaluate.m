function popu = gprls_evaluate(popuin,ixs,X,Y,Q,optv)
%Evaluates individulas and identificates their linear parameters
% popu = gprls_evaluate(popuin,ixs,X,Y,Q,optv)
...
for j = ixs,  
  ...
  %OLSQ   
  [vfsdel,err] = gprls_olsq(fs,X,Y,olslimit);
  tree.err = err; 
  vvdel = vv(vfsdel);
  ...
  %LSQ
  [mse,cfsq,theta] = gprls_lsq(fs,X,Y);
  fit = cfsq;
